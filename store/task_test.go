...
func prepareUser(ctx context.Context, t *testing.T, db Execer) entity.UserID {
	...
	if err != nil {
		t.Fatalf("get last insert id failed: %v", err)
	}
...
func prepareTasks(ctx context.Context, t *testing.T, con Execer) (entity.UserID, entity.Tasks) {
	...
	anotherUserID := prepareUser(ctx, t, con)
	...
	tasks := entity.Tasks{
		wants[0],
		&entity.Task{
			UserID:   anotherUserID, // 他のユーザーのタスク
			Title:    "not want task",
			Status:   "todo",
			Created:  c.Now(),
			Modified: c.Now(),
...
func TestRepository_AddTask(t *testing.T) {
	...
	mock.ExpectExec(
		...
		sqlmock.NewResult(int64(wantID), 1),
	)
